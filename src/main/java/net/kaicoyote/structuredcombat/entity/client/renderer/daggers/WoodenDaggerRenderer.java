...
@OnlyIn(Dist.CLIENT)
public class WoodenDaggerRenderer extends EntityRenderer<WoodenDaggerProjectileEntity>{
    private final DaggerModel model;
    public WoodenDaggerRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        this.model = new DaggerModel(pContext.bakeLayer(ModModelLayers.WOODEN_DAGGER));
    }
    ...
    public ResourceLocation getTextureLocation(WoodenDaggerProjectileEntity woodenDaggerProjectileEntity) {
        return new ResourceLocation(StructuredCombat.MOD_ID, "textures/entity/daggers/wooden_dagger.png");
    }
}
