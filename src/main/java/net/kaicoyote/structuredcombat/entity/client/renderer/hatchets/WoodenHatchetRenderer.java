...
import net.kaicoyote.structuredcombat.entity.custom.entities.daggers.WoodenDaggerProjectileEntity;
...
@OnlyIn(Dist.CLIENT)
public class WoodenHatchetRenderer extends EntityRenderer<WoodenHatchetProjectileEntity>{
    private final HatchetModel model;
    public WoodenHatchetRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        this.model = new HatchetModel(pContext.bakeLayer(ModModelLayers.WOODEN_HATCHET));
    }

    @Override
    public void render(WoodenHatchetProjectileEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
    ...
    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull WoodenHatchetProjectileEntity woodenDaggerProjectileEntity) {
        return new ResourceLocation(StructuredCombat.MOD_ID, "textures/entity/hatchets/wooden_hatchet.png");
    }
}
