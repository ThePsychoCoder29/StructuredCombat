...
import net.kaicoyote.structuredcombat.entity.custom.entities.spears.IronSpearProjectileEntity;
...
@OnlyIn(Dist.CLIENT)
public class IronSpearRenderer extends EntityRenderer<IronSpearProjectileEntity>{
    private final SpearWithGuardModel model;
    public IronSpearRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        this.model = new SpearWithGuardModel(pContext.bakeLayer(ModModelLayers.IRON_SPEAR));
    }

    @Override
    public void render(IronSpearProjectileEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, @NotNull MultiBufferSource pBuffer, int pPackedLight) {
    ...
    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull IronSpearProjectileEntity ironSpearProjectileEntity) {
        return new ResourceLocation(StructuredCombat.MOD_ID, "textures/entity/spears/iron_spear.png");
    }
}
