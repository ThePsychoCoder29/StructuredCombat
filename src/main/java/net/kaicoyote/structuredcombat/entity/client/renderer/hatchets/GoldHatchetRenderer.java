...
@OnlyIn(Dist.CLIENT)
public class GoldHatchetRenderer extends EntityRenderer<GoldHatchetProjectileEntity>{
    private final HatchetModel model;
    public GoldHatchetRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        this.model = new HatchetModel(pContext.bakeLayer(ModModelLayers.GOLD_HATCHET));
    }

    @Override
    public void render(GoldHatchetProjectileEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
    ...
    @Override
    public ResourceLocation getTextureLocation(GoldHatchetProjectileEntity goldHatchetProjectileEntity) {
        return new ResourceLocation(StructuredCombat.MOD_ID, "textures/entity/hatchets/gold_hatchet.png");
    }
}
